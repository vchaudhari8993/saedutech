...
namespace saEdu
{
    public partial class show_transaction_details : Form
    {
        public show_transaction_details()
        {
            InitializeComponent();
        }

        private void show_transaction_details_Load(object sender, EventArgs e)
        {
            JObject obj2;
            int counter1 = 0;
            int counter2 = 0;
            ...
            var httpWebRequest1 = (HttpWebRequest)WebRequest.Create(GlobalClass.url + "/show_all_transactions/");
            httpWebRequest1.ContentType = "application/json";
            httpWebRequest1.Method = "POST";
            httpWebRequest1.CookieContainer = new CookieContainer();
            httpWebRequest1.CookieContainer.Add(new Uri(GlobalClass.url + "/show_all_transactions/"), new Cookie("sessionid", GlobalClass.session));
            ...
            using (var streamReader1 = new StreamReader(httpResponse1.GetResponseStream()))
            {
                ...
                obj2 = JObject.Parse(Convert.ToString((JToken)(obj1["transactionList"][0])));
                //MessageBox.Show(Convert.ToString(obj2));
                counter1 = obj2.Count;
                if (counter1 > 0)
                {
        ...
        private void show_transaction_details_Paint(object sender, PaintEventArgs e)
        {
            for (int i = 0; i < show_transaction_grid.Rows.Count - 1; i++)
            {
                if (show_transaction_grid.Rows[i].Cells[1].Value.ToString() == "Contra")
                {
                    show_transaction_grid.Rows[i].DefaultCellStyle.BackColor = Color.Yellow;
                }
                else if (show_transaction_grid.Rows[i].Cells[1].Value.ToString() == "Payment")
                {
                    show_transaction_grid.Rows[i].DefaultCellStyle.BackColor = Color.Gray;
                }
            }
        }
    }
